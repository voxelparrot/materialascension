...
public class LootTableModification {
  ...
  static @NotNull ResourceLocation register(@NotNull String resourceName) {
    return register(new ResourceLocation(Constants.MA_ID, resourceName));
  }
    ...
    if (name.startsWith(prefix)) {
      ...
      if (BW_TABLES.containsKey(file)) {
        try {
          ...
          Constants.MA_LOG.warn("Failed to add to vanilla loot table: {}. This is most likely due to an incompatibility with another mod. Please report this on the Basic Weapons issue tracker, thank you.", name);
        }
      }
    }
  }
